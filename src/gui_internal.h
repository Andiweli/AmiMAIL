...
void set_string(struct Gadget *gadget, struct Window *window, const char *text);
void set_utf8_string(struct Gadget *gadget, struct Window *window, const char *utf8);
...
void header_to_local(const char *header, const char *fallback, char *local, size_t capacity);
...
void attach_listbrowser(struct Gadget *gadget, struct Window *window, struct List *list);
...
void sync_listbrowser_scroller(struct Window *window, struct Gadget *listbrowser, struct Gadget *scroller);
void handle_listbrowser_scroller(struct Window *window, struct Gadget *listbrowser, struct Gadget *scroller);
void sync_texteditor_scroller(struct Window *window, struct Gadget *editor, struct Gadget *scroller, ULONG fallback_entries, int reset_top);
void handle_texteditor_scroller(struct Window *window, struct Gadget *editor, struct Gadget *scroller, ULONG fallback_entries);

/* runtime/timer/iconify */
...
size_t update_labels_from_payload(AmgGui *gui,const unsigned char *payload,size_t length);
...
size_t message_uid_stats(const unsigned char *payload,size_t length,unsigned long baseline,unsigned long *max_uid,int *parse_error);
size_t message_unseen_count_from_payload(const unsigned char *payload,
                                         size_t length, int *parse_error);
size_t update_messages_from_payload(AmgGui *gui,const unsigned char *payload,size_t length,int *parse_error);
...
size_t merge_new_messages_from_payload(AmgGui *gui,const unsigned char *payload,size_t length,int *parse_error);
...
int display_message_payload(AmgGui *gui,const unsigned char *payload,size_t payload_length,AmgError *error);
void clear_current_message_payload(AmgGui *gui);
void retain_current_message_payload(AmgGui *gui,const AmgNetworkEvent *event);
void save_current_attachments(AmgGui *gui);
void sanitize_attachment_name(const char *name_utf8, char *name_local, size_t capacity);
int build_unique_attachment_path(const char *drawer, const char *name, char *path, size_t capacity);

/* compose/drafts */
void cleanup_draft_edit_files(DraftEditData *edit);
int prepare_draft_edit_payload(AmgGui *gui,const unsigned char *payload,size_t payload_length,unsigned long uid,const char *mailbox_utf8,DraftEditData *edit,AmgError *error);
int prepare_reply_payload(AmgGui *gui,const unsigned char *payload,size_t payload_length,AmgError *error);
int compose_dialog(AmgGui *gui, ComposeMode mode, DraftEditData *seed,
                   AmgError *error);

/* window */
int create_window(AmgGui *gui, AmgError *error);
void center_window_on_screen(struct Window *window);
void draw_embedded_banner_at(AmgGui *gui, struct Window *window,LONG left,LONG top,LONG available_width,LONG available_height);
...
int confirm_question_dialog_for_window(AmgGui *gui,struct Window *ref_window,const char *question,const char *note,LONG width);
...
int gui_contacts_select_emails(AmgGui *gui, struct Window *parent, struct Gadget *target, AmgError *error);
