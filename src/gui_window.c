...
static struct NewMenu menus[] = {
    ...
    {NM_ITEM, (STRPTR)"Contact management...", (STRPTR)"K", 0, 0, NULL},
    ...
    {NM_ITEM, (STRPTR)"Copy", (STRPTR)"C", NM_ITEMDISABLED, 0, NULL},
    {NM_ITEM, (STRPTR)"Cut", (STRPTR)"X", NM_ITEMDISABLED, 0, NULL},
    {NM_ITEM, (STRPTR)"Paste", (STRPTR)"V", NM_ITEMDISABLED, 0, NULL},
    {NM_ITEM, NM_BARLABEL, NULL, 0, 0, NULL},
    {NM_ITEM, (STRPTR)"Select All", (STRPTR)"A", NM_ITEMDISABLED, 0, NULL},
    {NM_ITEM, NM_BARLABEL, NULL, 0, 0, NULL},
...
static struct NewMenu compose_menus[] = {
    {NM_TITLE, (STRPTR)"File", NULL, 0, 0, NULL},
    {NM_ITEM, (STRPTR)"Account settings...", (STRPTR)"E", 0, 0, NULL},
    {NM_ITEM, NM_BARLABEL, NULL, 0, 0, NULL},
    {NM_ITEM, (STRPTR)"Contact management...", (STRPTR)"K", 0, 0, NULL},
...
static char reply_button_label[128];

static const char *reply_label_with_w_shortcut(const char *source)
{
    size_t used = 0U;
    int marked = 0;
    if (!source) source = "";
    while (*source && used + 1U < sizeof(reply_button_label)) {
        if (*source == '_') {
            ++source;
            continue;
        }
        if (!marked && (*source == 'w' || *source == 'W') &&
            used + 2U < sizeof(reply_button_label)) {
            reply_button_label[used++] = '_';
            marked = 1;
        }
        reply_button_label[used++] = *source++;
    }
    reply_button_label[used] = 0;
    return reply_button_label;
}

static void localize_menu_array(struct NewMenu *menu)
{
    if (!menu) return;
    menu[0].nm_Label=(STRPTR)T(MSG_FILE, "File");
    menu[1].nm_Label=(STRPTR)T(MSG_ACCOUNT_SETTINGS, "Account settings...");
    menu[3].nm_Label=(STRPTR)menu_contact_management_label;
    menu[4].nm_Label=(STRPTR)menu_signature_label;
    menu[6].nm_Label=(STRPTR)T(MSG_ABOUT_AMIMAIL_38B2, "About AmiMail...");
    menu[8].nm_Label=(STRPTR)T(MSG_QUIT, "Quit");
    menu[9].nm_Label=(STRPTR)T(MSG_EDIT_A8FE, "Edit");
    menu[10].nm_Label=(STRPTR)T(MSG_COPY, "Copy");
    menu[11].nm_Label=(STRPTR)T(MSG_CUT, "Cut");
    menu[12].nm_Label=(STRPTR)T(MSG_PASTE, "Paste");
    menu[14].nm_Label=(STRPTR)T(MSG_SELECT_ALL, "Select All");
    menu[16].nm_Label=(STRPTR)T(MSG_EMPTY_TRASH, "Empty Trash...");
    menu[17].nm_Label=(STRPTR)T(MSG_EMPTY_SPAM, "Empty Spam...");
}
...
static void localize_menus(void)
{
    ...
    localize_menu_array(menus);
    localize_menu_array(compose_menus);
}

struct NewMenu *gui_compose_menu_definition(void)
{
    localize_menus();
    return compose_menus;
}
...
static const unsigned char *app_header_chunk(const char id[4],
                                              size_t *length)
{
    size_t position = 12U;
    if (amg_header_iff_size < position ||
        memcmp(amg_header_iff, "FORM", 4U) ||
        memcmp(amg_header_iff + 8U, "ILBM", 4U))
        return NULL;
    while (position + 8U <= amg_header_iff_size) {
        size_t chunk_length =
            (size_t)banner_be32(amg_header_iff + position + 4U);
        size_t data_position = position + 8U;
        if (chunk_length > amg_header_iff_size - data_position) return NULL;
        if (!memcmp(amg_header_iff + position, id, 4U)) {
            *length = chunk_length;
            return amg_header_iff + data_position;
        }
        position = data_position + chunk_length + (chunk_length & 1U);
    }
    return NULL;
...
static void prepare_app_header_pens(AmgGui *gui)
{
    const unsigned char *palette;
    size_t palette_length = 0U;
    size_t i;
    if (!gui || !gui->screen || !gui->screen->ViewPort.ColorMap) return;
    palette = app_header_chunk("CMAP", &palette_length);
    for (i = 0; i < APP_HEADER_COLOR_COUNT; ++i) {
        unsigned char red = 0x99U, green = 0x99U, blue = 0x99U;
        LONG pen;
        if (palette && i * 3U + 2U < palette_length) {
            red = palette[i * 3U];
            green = palette[i * 3U + 1U];
        ...
        pen = ObtainBestPenA(
            ...
            (ULONG)blue * 0x01010101UL, NULL);
        if (pen >= 0) {
            gui->app_header_pens[i] = pen;
            gui->app_header_pen_owned[i] = 1U;
        } else {
            pen = FindColor(
                gui->screen->ViewPort.ColorMap,
                (ULONG)red * 0x01010101UL,
...
static void prepare_update_pen(AmgGui *gui)
{
    LONG pen;
    if (!gui || !gui->screen) return;
    /* Deep blue (#003366) stays readable on the banner's #999999 grey
...
static void draw_app_header_at(AmgGui *gui, struct Window *window,
                               LONG left, LONG top,
                               LONG available_width, LONG available_height)
{
    const unsigned char *header, *body;
    ...
    header = app_header_chunk("BMHD", &header_length);
    body = app_header_chunk("BODY", &body_length);
    if (!header || header_length < 20U || !body) return;
    width = banner_be16(header);
    height = banner_be16(header + 2U);
    planes = header[8U];
    ...
    for (y = 0; y < draw_height; ++y) {
        ...
        while (x < draw_width) {
            ...
            SetAPen(rastport, (ULONG)gui->app_header_pens[color]);
            RectFill(rastport, left + (LONG)run_start, top + (LONG)y,
                     left + (LONG)x - 1L, top + (LONG)y);
        }
    }
...
static void draw_banner(AmgGui *gui)
{
    ...
    if (right < left ||
        ...
        return;
    SetAPen(rastport, (ULONG)gui->banner_pens[7]);
    ...
    draw_app_header_at(gui, gui->window, logo_left, top,
...
/* The banner and version string are drawn directly into the window RastPort
 * rather than by ReAction gadgets. The main window uses Smart Refresh so
 * obscured pixels survive normal window overlap. This post-refresh hook
 * remains the redraw path for genuine refresh damage such as resizing. Do
 * not call RefreshGList() or any scrollbar synchronisation helpers here. */
...
/* The native ReAction layout.gadget owns the splitter's mouse tracking and
 * weight updates.  We only constrain the two panes to a sensible range and
 * persist the resulting percentage when the window closes. */
...
ULONG gui_mail_split_current_percent(const AmgGui *gui)
{
    ...
    ULONG top_height, bottom_height, total;
    ...
    return mail_split_clamp_percent((top_height * 100U + total / 2U) / total);
}
...
void gui_mail_split_update_limits(AmgGui *gui, int relayout)
{
    ...
    min_height = (usable_height + 2U) / 3U;
    max_height = (usable_height * 2U) / 3U;
...
{
    Object *banner_row;
    struct LayoutLimits labels_scroller_limits;
    ULONG labels_scroller_reserve_width = 16UL;
    ...
    banner_row = HGroupObject,
        ...
        LAYOUT_AddChild, VGroupObject,
            ...
            LAYOUT_AddChild,
                ...
                BUTTON_BackgroundPen, gui->banner_pens[7],
                BUTTON_FillTextPen, gui->text_pen,
                BUTTON_FillPen, gui->banner_pens[7],
            EndObject,
            CHILD_WeightedHeight, 0,
        EndObject,
    ...
    /* The lower folder browser has a separate vertical scroller while the
     * fixed-folder browser above it has none.  Ask the scroller itself for
     * its ReAction minimum domain *before* building the window and reserve
     * exactly that width beside the upper browser.  The lower scroller is a
     * zero-weight child, so layout.gadget assigns it this same minimum width.
     * This keeps both ListBrowser gadgets exactly equal in width without any
     * post-layout geometry reads or runtime LAYOUT_ModifyChild calls. */
    memset(&labels_scroller_limits, 0, sizeof(labels_scroller_limits));
    LayoutLimits(gui->labels_scroller, &labels_scroller_limits,
                 gui->screen ? gui->screen->RastPort.Font : NULL,
                 gui->screen);
    if (labels_scroller_limits.MinWidth > 0U)
        labels_scroller_reserve_width = labels_scroller_limits.MinWidth;
    ...
    gui->window_object = WindowObject,
        ...
        WINDOW_ParentGroup, VGroupObject,
            LAYOUT_SpaceOuter, FALSE,
            LAYOUT_SpaceInner, FALSE,
            /* Do not install a private Layout fill here.  window.class and
             * ReAction must remain responsible for the window backfill so
             * the user's configured background pattern is visible. */
            ...
            LAYOUT_AddChild, VGroupObject,
                ...
                /* Account tabs belong directly below the header. The outer
                 * layout spacing provides a small visual gap to the banner;
                 * the action toolbar follows underneath. */
                LAYOUT_AddChild,
                    gui->account_tabs_gadget = (struct Gadget *)NewObject(
                        CLICKTAB_GetClass(), NULL,
                        GA_ID, GID_ACCOUNT_TABS,
                        GA_RelVerify, TRUE,
                        ...
                        CLICKTAB_TabsOffsetAsLayoutSpacing, FALSE,
                        TAG_DONE),
                CHILD_WeightedHeight, 0,
                ...
                LAYOUT_AddChild, HGroupObject,
                    ...
                    LAYOUT_AddChild,
                        ...
                        GA_RelVerify, TRUE,
                        GA_ActivateKey,
                            window_current_mailbox_is_drafts(gui)
                                ? 0UL : (ULONG)(uintptr_t)"W",
                        GA_Text, window_current_mailbox_is_drafts(gui)
                            ? T(MSG_EDIT, "_Edit")
                            : reply_label_with_w_shortcut(
                                  T(MSG_REPLY, "Reply")),
                    EndObject,
                    CHILD_MinWidth, 92,
                    CHILD_WeightedWidth, 100,
            ...
            LAYOUT_AddChild, HGroupObject,
                    ...
                    VGroupObject,
                        ...
                        LAYOUT_AddChild, HGroupObject,
                            ...
                            LAYOUT_AddChild, HGroupObject,
                                ...
                                LAYOUT_SpaceInner, FALSE,
                            EndObject,
                            /* Reserve exactly the minimum domain width that
                             * ReAction assigns to labels_scroller below. */
                            CHILD_MinWidth, labels_scroller_reserve_width,
                            CHILD_WeightedWidth, 0,
                        EndObject,
                        CHILD_WeightedHeight, 0,
                ...
                LAYOUT_AddChild,
                    ...
                    LAYOUT_SpaceOuter, FALSE,
                    LAYOUT_SpaceInner, TRUE,
                    /* Keep this group transparent as well.  Otherwise its
                     * solid Layout backfill would cover the window pattern
                     * in the gaps between the mail list and preview. */
                    LAYOUT_AddChild,
                    ...
                    LAYOUT_AddChild,
                        ...
                        LAYOUT_AddChild, HGroupObject,
                            LAYOUT_SpaceOuter, FALSE,
                            LAYOUT_AddChild,
                                gui->reply_status_gadget =
                                    (struct Gadget *)ButtonObject,
                                    GA_ReadOnly, TRUE,
                                    GA_Text, gui->reply_status_text[0],
                                    BUTTON_DomainString, " ",
    ...
    if (!connect_texteditor_scroller(gui->preview_gadget,
        ...
        amg_error_set(error, AMG_ERR_MEMORY,
                      T(MSG_SCROLLBAR_COULD_NOT_BE_CREATED,
                        "Scrollbar could not be created."));
        return AMG_ERR_MEMORY;
    }
    return AMG_OK;
}
