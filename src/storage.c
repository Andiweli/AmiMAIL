...
#define STORAGE_ITERATIONS_CURRENT 5000UL
#define STORAGE_ITERATIONS_LEGACY 100000UL
#define STORAGE_ITERATIONS_MIN 1000UL
#define STORAGE_ITERATIONS_MAX 1000000UL
                           ...
                           AmgBuffer *cipher, AmgError *error)
{
    ...
    if (PKCS5_PBKDF2_HMAC(master, (int)strlen(master), salt, 16,
                          (int)STORAGE_ITERATIONS_CURRENT,
                          EVP_sha256(), 32, key) != 1)
        goto done;
                              ...
                              AmgError *error)
{
    ...
    if (iterations < STORAGE_ITERATIONS_MIN ||
        iterations > STORAGE_ITERATIONS_MAX)
...
static int decrypt_secrets(const char *master, const unsigned char *cipher,
                           ...
                           const unsigned char iv[12],
                           const unsigned char tag[16],
                           unsigned long iterations, AmgBuffer *plain,
                           AmgError *error)
{
    unsigned char key[SESSION_KEY_SIZE];
    int result;
    memset(key, 0, sizeof(key));
    result = derive_storage_key(master, salt, iterations, key, error);
    if (result == AMG_OK)
...
static unsigned long storage_iterations_from_data(const char *data)
{
    char value[64];
    char *end = NULL;
    unsigned long iterations;
    ...
    if (!field(data, "iterations", value, sizeof(value)))
        return STORAGE_ITERATIONS_LEGACY;

    iterations = strtoul(value, &end, 10);
    while (end && (*end == ' ' || *end == '\t' || *end == '\r')) ++end;
    if (!value[0] || (end && *end) ||
        iterations < STORAGE_ITERATIONS_MIN ||
        iterations > STORAGE_ITERATIONS_MAX)
        return 0UL;
    return iterations;
}
...
static int read_account_iterations(const char *path,
                                   unsigned long *iterations,
                                   AmgError *error)
{
    size_t length;
    ...
    if (!path || !iterations) return AMG_ERR_ARGUMENT;
    *iterations = 0UL;
    data = read_all(path, &length);
    (void)length;
    if (!data) return AMG_ERR_IO;
    ...
    if (storage_header_version(data) &&
        field(data, "secrets", value, sizeof(value)) &&
        !strcmp(value, "aes-256-gcm")) {
        parsed = storage_iterations_from_data(data);
        if (parsed) {
            *iterations = parsed;
            result = AMG_OK;
        }
...
int amg_storage_account_needs_kdf_refresh(const char *path)
{
    ...
    if (read_account_iterations(path, &iterations, &error) != AMG_OK)
        return 0;
    return iterations == STORAGE_ITERATIONS_LEGACY &&
           STORAGE_ITERATIONS_CURRENT != STORAGE_ITERATIONS_LEGACY;
}
                                  ...
                                  AmgError *error)
{
    ...
    if (result == AMG_OK)
        result = derive_storage_key(master_password, salt, iterations,
                                    key, error);
    if (result == AMG_OK) {
...
#undef LOAD_HEX_STRING
    ...
    if (field(data, "secrets", value, sizeof(value)) &&
        ...
        unsigned long iterations = storage_iterations_from_data(data);
        ...
        if (!iterations ||
            ...
            result = decrypt_secrets(master_password, cipher.data,
                                     ...
                                     tag.data, iterations, &plain, error);
        } else {
            result = AMG_ERR_AUTH;
        }
#else
