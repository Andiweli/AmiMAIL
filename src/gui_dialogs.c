...
static int persistent_unlock_enabled(void)
{
    BPTR lock = Lock((CONST_STRPTR)PERSISTENT_KEY_PATH, ACCESS_READ);
    if (!lock) return 0;
    UnLock(lock);
    return 1;
}
                    ...
                    case WMHI_GADGETUP:
                        ...
                        if ((result & WMHI_GADGETMASK) == GID_ACCOUNT_CANCEL) {
                            ...
                            if (amg_storage_load_account(
                                ...
                                if (amg_storage_account_needs_kdf_refresh(
                                        ACCOUNT_PATH)) {
                                    (void)amg_storage_save_account(
 ...
 int account_dialog(AmgGui *gui, AmgError *error)
{
    ...
    struct Gadget *no_master_prompt_gadget;
    ...
    int persistent_cache_warning = 0;
    int network_was_running = amg_network_is_running(gui->network);
    int network_settings_changed = 0;
    int periodic_fetch_changed = 0;
    int can_unlock_existing = account_file_exists();
    int persistent_unlock = persistent_unlock_enabled();
        ...
        WINDOW_ParentGroup, VGroupObject,
            ...
            LAYOUT_AddChild, VGroupObject,
                ...
                LAYOUT_AddChild, HGroupObject,
                    ...
                    CHILD_WeightedWidth, 0,
                    LAYOUT_AddChild,
                        no_master_prompt_gadget =
                            (struct Gadget *)ButtonObject,
                            GA_ID, GID_ACCOUNT_NO_MASTER_PROMPT,
                            GA_RelVerify, TRUE,
                            GA_Selected,
                                persistent_unlock ? TRUE : FALSE,
                            BUTTON_AutoButton, BAG_CHECKBOX,
                            BUTTON_PushButton, TRUE,
                        EndObject,
                    CHILD_MinWidth, 24,
                                    ...
                                    if (amg_storage_save_account(
                                        ...
                                        memset(&session_error, 0,
                                               sizeof(session_error));
                                                                                if (persistent_unlock_enabled()) {
                                            AmgError persistent_error;
                                            memset(&persistent_error, 0,
                                                   sizeof(persistent_error));
                                            if (amg_storage_cache_session_key(
                                                    ACCOUNT_PATH,
                                                    PERSISTENT_KEY_PATH,
                                                    master,
                                                    &persistent_error) != AMG_OK)
                                                persistent_cache_warning = 1;
                                        }
if (amg_storage_cache_session_key(
                                                ACCOUNT_PATH,
                            ...
                            case GID_ACCOUNT_SAVE:
                            {
                                ...
                                GetAttr(
                                    GA_Selected,
                                    (Object *)no_master_prompt_gadget,
                                    &no_master_prompt);
                                ...
                                if (no_master_prompt) {
                                    ...
                                    amg_storage_forget_session_key(
                                        PERSISTENT_KEY_PATH);
                                }
